...
import { PlantsDataProps } from "../types";

export async function fetchDataPlants(): Promise<PlantsDataProps[] | string> {
  // const apiKey = "sk-nMOS66022a3d3a15e4864";
  // const apiUrl = `https://perenual.com/api/species-list?key=${apiKey}`;

  try {
    // const response: AxiosResponse = await axios.get(apiUrl);
    // if (response.status === 409) {
    //   throw Error("Erro validação");
    // }
    const data: PlantsDataProps[] = [
      {
        id: 1,
